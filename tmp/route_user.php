...
if(!empty($kv_vcode['vcode_user_login_on'])) {
	...
	strtolower($vcode_post) != strtolower($vcode_sess) AND message('vcode', '验证码不正确');
		...
		unset($_SESSION['vcode']);user_update_group($uid);
...
user_update($uid, array('avatar' => $time));unset($_SESSION['vcode']);
unset($_SESSION['vcode_initpw_ok']);	ipaccess_inc($longip, 'users');
